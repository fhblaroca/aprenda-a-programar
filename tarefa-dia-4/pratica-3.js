...
for (item of receita) {
    TotalSemGorjeta += item.valor
}
console.log("Valor total de vendas sem gorjeta: ", TotalSemGorjeta)

//Exercício 2: Qual o valor total de vendas incluindo a gorjeta?
let TotalComGorjeta = 0
for (item of receita) {
    TotalComGorjeta += item.valor + item.gorjeta
}
console.log("Valor total de vendas com gorjeta: ", TotalComGorjeta)

//Exercício 3: Quantas vendas foram abaixo de 50 reais?
let vendas50menos = 0
for (item of receita) {
    let valor1venda = item.valor + item.gorjeta
    if (valor1venda < 50)
        vendas50menos++
}
console.log("Número de vendas abaixo de 50 reais: ", vendas50menos)